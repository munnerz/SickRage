...
# @OneRunPP()
def process_dir(process_path, nzbName=None, process_method=None, force=False, is_priority=None, delete_on=False, failed=False, mode="auto"):
    """
    ...
    path, dirs, files = get_path_dir_files(process_path, nzbName, mode)
    ...
    for curDir in [x for x in dirs if validate_dir(path, x, nzbNameOriginal, failed, result)]:
        result.result = True

        for current_directory, directory_names, file_names in ek(os.walk, ek(os.path.join, path, curDir), topdown=False):

            if not validate_dir(path, current_directory, nzbNameOriginal, failed, result):
                continue
            ...
            if not postpone:
                ...
                # Don't Link media when the media is extracted from a rar in the same path
                if process_method in (u'hardlink', u'symlink') and videoInRar:
                    process_media(current_directory, videoInRar, nzbName, u'move', force, is_priority, result)
                    process_media(current_directory, set(videoFiles) - set(videoInRar), nzbName, process_method, force,
                                  is_priority, result)
                    delete_files(current_directory, rarContent, result)
                elif sickbeard.DELRARCONTENTS and videoInRar:
                    process_media(current_directory, videoInRar, nzbName, process_method, force, is_priority, result)
                    process_media(current_directory, set(videoFiles) - set(videoInRar), nzbName, process_method, force,
                                  is_priority, result)
                    delete_files(current_directory, rarContent, result, True)
                else:
                    process_media(current_directory, videoFiles, nzbName, process_method, force, is_priority, result)
...
def validate_dir(path, process_path, nzbNameOriginal, failed, result):  # pylint: disable=too-many-locals,too-many-branches,too-many-return-statements
    """
    ...
    if failed:
        process_failed(ek(os.path.join, path, process_path), nzbNameOriginal, result)
...
def process_media(process_path, videoFiles, nzbName, process_method, force, is_priority, result):  # pylint: disable=too-many-arguments
    """
...
def get_path_dir_files(process_path, nzbName, mode):
    """
    Get files in a path
    ...
    if process_path == sickbeard.TV_DOWNLOAD_DIR and not nzbName or mode == u"manual":  # Scheduled Post Processing Active
        ...
        if not (nzbName is None or nzbName.endswith(u'.nzb')) and ek(os.path.isfile, ek(os.path.join, process_path, nzbName)):  # For single torrent file without Dir
            dirs = []
            files = [ek(os.path.join, process_path, nzbName)]
        else:
            dirs = [dirs]
            files = []
...
def process_failed(process_path, nzbName, result):
    ...
    if sickbeard.USE_FAILED_DOWNLOADS:
        ...
        try:
            processor = failedProcessor.FailedProcessor(process_path, nzbName)
            result.result = processor.process()
            process_fail_message = u""
        ...
        if result.result:
            result.output += log_helper(u"Failed Download Processing succeeded: ({0}, {1})".format(nzbName, process_path))
        else:
            result.output += log_helper(u"Failed Download Processing failed: ({0}, {1}): {2}".format(nzbName, process_path, process_fail_message), logger.WARNING)
